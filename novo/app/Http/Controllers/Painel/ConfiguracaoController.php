    ...
    public function salvar_popup(){
        ...
        if(isset($_FILES["img_banner"])){
            ...
            if($varArquivo != ''){
                ...
                $configuracao->popup_img = $arquivo_nome_final;
            }

        }
        
        
        $configuracao->popup = Request::input('popup');
        $configuracao->popup_url = Request::input('popup_url');
        $configuracao->popup_altura = Request::input('popup_altura');
        $configuracao->popup_largura = Request::input('popup_largura');
        
//        die("<PRE>" . print_r($configuracao,1));
        $configuracao->save();
        
        Session::put('status.msg', 'Configurações do popup salvas com sucesso!');
        return redirect('/painel/sistema');
